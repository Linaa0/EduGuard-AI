    ...
    <div className="p-6 sm:p-8 space-y-8 bg-slate-50 min-h-full">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-4 border-b border-slate-200">
        <div>
          <div className="flex items-center gap-2">
            <h1 className="text-2xl sm:text-3xl font-extrabold text-slate-900 tracking-tight">AI Assessment Agent</h1>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
                    ...
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-2.5">
                        <div className={`w-5 h-5 rounded flex items-center justify-center font-mono text-[11px] font-bold ${
                          isDone ? 'bg-emerald-600 text-white' : isCurrent ? 'bg-blue-600 text-white animate-pulse' : 'bg-slate-800 text-slate-500'
                        }`}>
                          {isDone ? '✓' : idx + 1}
                        </div>
        ...
        <div className="lg:col-span-5 space-y-6">
          ...
          {/* Top Result Card */}
          <div className="p-6 rounded-2xl bg-white border border-slate-200 shadow-subtle space-y-5">
            <div className="flex items-center justify-between pb-3 border-b border-slate-100">
              <div>
                <span className="text-[10px] font-bold uppercase tracking-wider text-slate-400">AI Assessment Result</span>
                <h3 className="text-base font-bold text-slate-900">Climate Change Essay</h3>
              </div>
            ...
            <div className="text-center py-2 bg-slate-50 rounded-xl border border-slate-100">
              <span className="text-xs text-slate-500 uppercase tracking-wider font-semibold">Suggested Score</span>
            ...
            <div className="space-y-3 pt-1">
              <span className="text-xs font-bold text-slate-700 uppercase tracking-wider block">Criterion-Level Evaluation</span>
              ...
              {criteriaBreakdown.map((crit) => (
                <div key={crit.name} className="p-3 rounded-xl bg-slate-50 border border-slate-200/80 space-y-1.5">
                  <div className="flex items-center justify-between text-xs">
                    ...
                    <span className="font-mono font-bold text-slate-900">{crit.score} / {crit.max}</span>
                  </div>
