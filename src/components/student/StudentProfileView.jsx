  ...
  return (
    <div className="p-6 sm:p-8 space-y-8 bg-slate-50 min-h-full">
      ...
      <div className="pb-4 border-b border-slate-200">
        <h1 className="text-2xl sm:text-3xl font-extrabold text-slate-900 tracking-tight">Academic Student Profile</h1>
        <p className="text-xs sm:text-sm text-slate-500 mt-1">Official university academic record and enrolled modules</p>
      </div>
        ...
        <div className="p-6 rounded-2xl bg-white border border-slate-200 shadow-subtle space-y-4">
          ...
          <div className="pt-3 border-t border-slate-100 space-y-2 text-xs">
            ...
            <div className="flex justify-between text-slate-600">
              ...
              <span className="font-mono font-bold text-slate-900">{student.gpa} / 4.0</span>
            </div>
            <div className="flex justify-between text-slate-600">
              <span>Cohort Year:</span>
              <span className="font-bold text-slate-800">{student.year}</span>
            </div>
          </div>
        </div>

        {/* Enrolled Courses */}
        <div className="lg:col-span-2 p-6 rounded-2xl bg-white border border-slate-200 shadow-subtle space-y-4">
          <h3 className="text-sm font-bold text-slate-900">Enrolled Courses & Academic Standings</h3>

          <div className="space-y-3">
            {courses.map((c) => (
              <div key={c.code} className="p-3.5 rounded-xl bg-slate-50 border border-slate-200/80 flex items-center justify-between">
                <div>
                  <span className="text-[10px] font-mono font-bold px-2 py-0.5 rounded bg-blue-100 text-blue-800 border border-blue-200">
                    {c.code}
                  </span>
                  <h4 className="text-xs font-bold text-slate-900 mt-1">{c.name}</h4>
                  <p className="text-[11px] text-slate-500">Instructor: {c.lecturer}</p>
                </div>
