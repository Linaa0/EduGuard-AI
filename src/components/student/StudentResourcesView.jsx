  ...
  return (
    <div className="p-6 sm:p-8 space-y-8 bg-slate-50 min-h-full">
      ...
      <div className="pb-4 border-b border-slate-200">
        <h1 className="text-2xl sm:text-3xl font-extrabold text-slate-900 tracking-tight">Academic Learning Resources</h1>
        <p className="text-xs sm:text-sm text-slate-500 mt-1">Curated university reading materials, methodology guides, and reference manuals</p>
      </div>

      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        {studentPortalData.learningResources.map((res) => (
          <div key={res.id} className="p-6 rounded-2xl bg-white border border-slate-200 shadow-subtle flex flex-col justify-between space-y-4 hover:border-blue-500/50 transition-all group">
            <div className="space-y-3">
              <span className="text-[10px] font-mono text-blue-600 bg-blue-50 px-2 py-0.5 rounded border border-blue-200 font-semibold">
                {res.type}
              </span>
              <h3 className="text-base font-bold text-slate-900 group-hover:text-blue-600 transition-colors">{res.title}</h3>
